...
// Zod schema for update lesson form validation (same as createLesson)
...
function UpdateLesson({ lesson, onClose, onUpdateSuccess }: UpdateLessonProps) {
  // State management
  const [isLoading, setIsLoading] = useState(false);
  const [lessonsInput, setLessonsInput] = useState("");
  const [showConfirmDialog, setShowConfirmDialog] = useState(false);
  const [formData, setFormData] = useState<UpdateLessonFormData | null>(null);
  ...
  const {
    ...
    defaultValues: {
      ...
      tag: lesson.tag,
      lessonsCovered: lesson.lessonsCovered,
    },
  });
  ...
  /**
   * Confirm update function - actually performs the API call
   */
  const confirmUpdate = async () => {
    ...
    try {
      ...
      await axiosClient.patch(`/updateLesson/${lesson._id}`, lessonData);
      ...
      if (axios.isAxiosError(error)) {
        ...
        toast.error(message, { id: "update-lesson" });
      } else {
        toast.error("An unexpected error occurred", { id: "update-lesson" });
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white/90 rounded-xl shadow-lg p-6 sm:p-8 max-w-xl w-full max-h-[90vh] overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Update Lesson</h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Button
                type="button"
                onClick={onClose}
                className="flex-1 h-11 sm:h-12 text-sm sm:text-base text-gray-700 bg-gray-200 hover:bg-gray-300 font-semibold"
              >
                Cancel
              </Button>
              <Button
                type="submit"
                className="flex-1 h-11 sm:h-12 text-sm sm:text-base text-white font-semibold bg-primary hover:bg-primary/90"
              >
                Review Changes
              </Button>
            </div>
      ...
      {showConfirmDialog && (
        <UpdateConfirmDialog
          ...
          lessonTitle={formData?.title || lesson.title}
          isLoading={isLoading}
        />
      )}
    </>
